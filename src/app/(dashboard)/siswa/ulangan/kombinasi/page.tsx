...
import {
  ...
  SOAL_DATA,
  SoalKepahaman,
} from "@/components/materi/sections/kombinasi/latihan";
...
// ── Page ─────────────────────────────────────────────────────────────────────
export default function AsesmenKombinasiPage() {
  ...
  const { isRestoring, saveAllDrafts } = useDraftSaver({
    ...
    onRestore: useCallback((restored: DraftAnswerPair[]) => { setAnswers(restored); }, []),
  });

  useEffect(() => { setLocked(phase === "active"); return () => setLocked(false); }, [phase, setLocked]);

  useEffect(() => {
    if (phase !== "active") return;
    timerRef.current = setInterval(() => {
      setTimeLeft((t) => {
        if (t <= 1) { clearInterval(timerRef.current!); setTimeout(() => handleSubmitRef.current(), 0); return 0; }
        return t - 1;
      });
    }, 1000);
  ...
  async function handleStart() {
    const dt = getDeviceType(); setDeviceType(dt);
    try {
      const res = await fetch("/api/asesmen-formatif/start-attempt", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ module_slug: "kombinasi", device_type: dt }),
      });
      if (!res.ok) {
  ...
  async function handleSubmit() {
    ...
    hasSubmittedRef.current = true; setIsSubmitting(true); setSubmitError(null);
    ...
    try {
      ...
      try {
        const evalRes = await fetch("/api/asesmen-formatif/evaluate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ submission_id: submissionData.submission_id, module_slug: "kombinasi" }) });
        if (!evalRes.ok) { const evalErr = await evalRes.json().catch(() => ({ error: "Unknown error" })); throw new Error(evalErr.error ?? "Gagal mengevaluasi jawaban"); }
        const evalData: EvaluationResult = await evalRes.json(); setEvaluationResult(evalData); setPhase("results");
      } catch (evalErr: unknown) { setEvalError(evalErr instanceof Error ? evalErr.message : "Gagal mengevaluasi jawaban"); setPhase("submitted"); }
    } catch (err: unknown) { setSubmitError(err instanceof Error ? err.message : "Gagal menyimpan jawaban. Silakan coba lagi."); hasSubmittedRef.current = false; setIsSubmitting(false); return; } finally { setIsSubmitting(false); }
  }
  ...
  // ── Loading ───────────────────────────────────────────────────────────────
  if (accessAllowed === null) return <div style={{ maxWidth: 700, margin: "0 auto", padding: "80px 24px", textAlign: "center" }}><p style={{ fontSize: 15, color: "#6b8f6d" }}>Memeriksa akses latihan...</p></div>;
  if (!accessAllowed) return <LockedScreen missingSections={missingSections} summary={accessSummary} />;
...
// ── Locked Screen ────────────────────────────────────────────────────────────
function LockedScreen({ missingSections, summary }: { missingSections: Array<{ conceptId: string; section: string }>; summary: { totalRequired: number; completed: number; missing: number } | null }) {
  const grouped = new Map<string, string[]>();
  for (const ms of missingSections) { const list = grouped.get(ms.conceptId); if (list) list.push(ms.section); else grouped.set(ms.conceptId, [ms.section]); }
  const hasDetail = missingSections.length > 0 && summary !== null;
  const progressPct = summary && summary.totalRequired > 0 ? Math.round((summary.completed / summary.totalRequired) * 100) : 0;

  return (
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "40px 24px" }}>
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        <div style={{ width: 72, height: 72, borderRadius: "50%", backgroundColor: "#fff5f0", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
          <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#b45309" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
        </div>
        <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#b45309", opacity: 0.7, margin: "0 0 6px" }}>Akses Terkunci</p>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#7c2d12", margin: "0 0 8px" }}>Selesaikan Materi Terlebih Dahulu</h1>
        <p style={{ fontSize: 14, color: "#9a7b5c", margin: 0, lineHeight: 1.7 }}>Kamu harus menyelesaikan seluruh section pada materi Kombinasi sebelum dapat mengerjakan latihan formatif ini.</p>
      </div>
      {summary && (
        <div style={{ backgroundColor: "#fffbeb", borderRadius: 12, border: "1.5px solid #fde68a", padding: "14px 18px", marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#92400e" }}>Progress Materi</span>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#92400e" }}>{summary.completed}/{summary.totalRequired} selesai</span>
          </div>
          <div style={{ height: 8, backgroundColor: "#fde68a", borderRadius: 99, overflow: "hidden" }}><div style={{ height: "100%", backgroundColor: "#d97706", borderRadius: 99, width: `${progressPct}%`, transition: "width 0.5s ease" }} /></div>
        </div>
      )}
      {hasDetail ? (
        <div style={{ backgroundColor: "#fff", borderRadius: 12, border: "1.5px solid #e2ede2", padding: "18px 20px", marginBottom: 24 }}>
          <h3 style={{ fontSize: 12, fontWeight: 700, color: "#346739", margin: "0 0 12px", textTransform: "uppercase", letterSpacing: "0.1em" }}>Section yang Belum Selesai</h3>
          {Array.from(grouped.entries()).map(([conceptId, sections]) => (
            <div key={conceptId} style={{ marginBottom: 12 }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: "#1a3d1c", margin: "0 0 6px", textTransform: "uppercase", letterSpacing: "0.06em" }}>{CONCEPT_LABELS[conceptId] ?? conceptId}</p>
              <ul style={{ listStyle: "none", padding: "0 0 0 4px", margin: 0, display: "flex", flexDirection: "column", gap: 6 }}>
                {sections.map((section) => (<li key={section} style={{ fontSize: 13, color: "#5a7d5c", display: "flex", alignItems: "center", gap: 8 }}><span style={{ width: 6, height: 6, borderRadius: "50%", backgroundColor: "#d97706", flexShrink: 0 }} />{SECTION_LABELS[section] ?? section}</li>))}
              </ul>
            </div>
          ))}
        </div>
      ) : (
        <div style={{ backgroundColor: "#fff5f5", borderRadius: 12, border: "1.5px solid #fecaca", padding: "14px 18px", marginBottom: 24, textAlign: "center" }}><p style={{ fontSize: 13, color: "#b91c1c", margin: 0 }}>Tidak dapat memeriksa status materi saat ini. Silakan coba lagi beberapa saat.</p></div>
      )}
      <div style={{ textAlign: "center" }}>
        <Link href="/siswa/materi/kombinasi" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 32px", backgroundColor: "#346739", color: "#fff", borderRadius: 10, fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>Kembali ke Materi
        </Link>
        <p style={{ marginTop: 10, fontSize: 12, color: "#9aada0" }}>Selesaikan semua section di atas, lalu kembali ke halaman ini untuk memulai latihan pemahaman.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto", padding: "36px 24px" }}>
      ...
      <div style={{ backgroundColor: "#f8fdf8", borderRadius: 12, border: "1px solid #d4e8d4", padding: "18px 20px", marginBottom: 16 }}>
        <h3 style={{ fontSize: 12, fontWeight: 700, color: "#346739", margin: "0 0 12px", textTransform: "uppercase", letterSpacing: "0.1em" }}>Petunjuk Pengerjaan</h3>
        <ol style={{ paddingLeft: 20, margin: 0 }}>{PETUNJUK.map((p, i) => (<li key={i} style={{ fontSize: 14, color: "#3b5e3d", lineHeight: 1.7, marginBottom: i < PETUNJUK.length - 1 ? 6 : 0 }}>{p}</li>))}</ol>
      </div>
...
function ActiveScreen({ answers, timeLeft, onUpdateAnswer, onSubmit, isSubmitting, submitError, onDismissSubmitError, attemptId, deviceType, remainingQuestions }: {
  ...
  const registerPasteForQuestion = useCallback((idx: number, caraEl: HTMLTextAreaElement | null, jawabanEl: HTMLInputElement | null) => {
    const prev = pasteTargetRefs.current.get(idx); if (prev) { integrity.registerPasteTarget(`cara_hitung_${idx + 1}`, null); integrity.registerPasteTarget(`jawaban_akhir_${idx + 1}`, null); }
    pasteTargetRefs.current.set(idx, { cara: caraEl, jawaban: jawabanEl }); integrity.registerPasteTarget(`cara_hitung_${idx + 1}`, caraEl); integrity.registerPasteTarget(`jawaban_akhir_${idx + 1}`, jawabanEl);
  }, [integrity]);

  return (
    <div style={{ maxWidth: 700, margin: "0 auto", padding: "0 24px 48px" }}>
      {integrity.activeToast && <IntegrityToast toast={integrity.activeToast} onDismiss={() => {}} />}
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, backgroundColor: "#fff", borderBottom: "1.5px solid #e2ede2", paddingTop: 10, marginBottom: 28 }}>
        ...
        <div style={{ height: 3, backgroundColor: "#e2ede2", borderRadius: 99, overflow: "hidden" }}><div style={{ height: "100%", backgroundColor: color, borderRadius: 99, width: `${pct}%`, transition: "width 1s linear, background-color 0.5s" }} /></div>
      </div>
          ...
          <div key={soal.question_number} id={`soal-${soal.question_number}`} style={{ backgroundColor: "#fff", borderRadius: 14, border: `2px solid ${answered ? "#346739" : "#e2ede2"}`, overflow: "hidden", transition: "border-color 0.2s ease" }}>
            ...
            <div style={{ padding: "18px 20px" }}>
              ...
              <div>
                ...
                <NumericAnswerInput id={`jawaban-akhir-${i}`} ref={(el) => { const prev = pasteTargetRefs.current.get(i); registerPasteForQuestion(i, prev?.cara ?? null, el); }} value={answers[i].jawaban_akhir} onChange={(val) => onUpdateAnswer(i, "jawaban_akhir", val)} placeholder="Tulis angka saja..." className="w-full px-3 py-2.5 rounded-lg text-sm text-[#2C2C2A] border border-[#e8d4e8] bg-[#fdf8fd] placeholder:text-[#c4a8c2] focus:outline-none focus:ring-2 focus:ring-[#663362]/25 focus:border-[#663362] transition-all" style={{ fontFamily: "inherit" }} />
              </div>
            </div>
          </div>
        );})}
      ...
      <div style={{ marginTop: 32, paddingTop: 20, borderTop: "1.5px solid #e2ede2" }}>
        {submitError && <div style={{ backgroundColor: "#fff5f5", borderRadius: 10, border: "1.5px solid #fecaca", padding: "12px 16px", marginBottom: 16, textAlign: "center" }}><p style={{ fontSize: 13, color: "#b91c1c", margin: 0 }}>{submitError}</p><button onClick={onDismissSubmitError} style={{ marginTop: 10, padding: "8px 20px", borderRadius: 8, border: "1.5px solid #fecaca", backgroundColor: "#fff", fontSize: 13, fontWeight: 600, color: "#b91c1c", cursor: "pointer" }}>Coba Lagi</button></div>}
        {!showConfirm ? (
          <div style={{ textAlign: "center" }}><p style={{ fontSize: 13, color: "#6b8f6d", marginBottom: 12 }}>Sudah yakin dengan jawabanmu? Setelah submit, jawaban akan langsung dievaluasi.</p>
            <button onClick={() => setShowConfirm(true)} disabled={isSubmitting} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 32px", backgroundColor: isSubmitting ? "#9aada0" : "#346739", color: "#fff", borderRadius: 10, fontSize: 14, fontWeight: 700, border: "none", cursor: isSubmitting ? "not-allowed" : "pointer", opacity: isSubmitting ? 0.7 : 1 }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>{isSubmitting ? "Mengevaluasi..." : "Submit Jawaban"}
            </button>
          </div>
        ) : (
...
function SubmittedScreen({ answers, remainingQuestions }: { answers: AnswerPair[]; remainingQuestions: number[] }) {
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "40px 24px 48px" }}>
      <div style={{ textAlign: "center", marginBottom: 32 }}>
        ...
        <div style={{ marginTop: 20, backgroundColor: "#f0faf0", borderRadius: 10, border: "1px solid #c3e6c3", padding: "14px 18px", textAlign: "left" }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            <div><p style={{ fontSize: 13, fontWeight: 700, color: "#1a3d1c", margin: "0 0 4px" }}>Jawaban sudah tersimpan</p><p style={{ fontSize: 12, color: "#5a7d5c", margin: 0, lineHeight: 1.6 }}>Jawabanmu sedang dilakukan proses koreksi. Kembali lagi ke menu latihan setelah beberapa jam untuk mendapatkan nilai hasil latihan.</p></div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 32, textAlign: "center" }}><Link href="/siswa/ulangan" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 28px", backgroundColor: "#346739", color: "#fff", borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>← Kembali ke Daftar Latihan</Link></div>
    </div>
  );
}
...
function ResultsScreen({ evaluationResult, answers, evalError, onRetry, remainingQuestions: _remainingQuestions }: { evaluationResult: EvaluationResult | null; answers: AnswerPair[]; evalError: string | null; onRetry: () => void; remainingQuestions: number[] }) {
  if (evalError) return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "60px 24px 48px", textAlign: "center" }}>
      ...
      <button onClick={onRetry} style={{ padding: "10px 24px", borderRadius: 8, border: "1.5px solid #d4e8d4", backgroundColor: "#fff", fontSize: 14, fontWeight: 600, color: "#346739", cursor: "pointer" }}>Lihat Ringkasan Jawaban</button>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 24px 48px" }}>
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        <div style={{ width: 120, height: 120, borderRadius: "50%", backgroundColor: scoreBg, border: `3px solid ${scoreBorder}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
          <span style={{ fontSize: 36, fontWeight: 800, color: scoreColor, lineHeight: 1 }}>{total_score}</span><span style={{ fontSize: 11, fontWeight: 600, color: scoreColor, opacity: 0.7 }}>dari 100</span>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 32 }}>
        {per_question.map((pq) => {
          ...
          return (
            <div key={pq.question_number} style={{ backgroundColor: itemBg, borderRadius: 12, border: `1.5px solid ${itemBorder}`, padding: "16px 18px" }}>
      ...
      <div style={{ textAlign: "center", display: "flex", gap: 12, justifyContent: "center" }}><Link href="/siswa/ulangan" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 28px", backgroundColor: "#346739", color: "#fff", borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>← Kembali ke Daftar Latihan</Link></div>
    </div>
  );
}
